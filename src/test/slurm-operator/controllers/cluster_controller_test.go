...
func TestReconcileCreateUpdateRestartAndRepair(t *testing.T) {
	...
	if controllers.Spec.Template.Spec.AutomountServiceAccountToken == nil || *controllers.Spec.Template.Spec.AutomountServiceAccountToken {
		t.Fatal("slurmctld has a Kubernetes service-account token")
	}
